class MyHeader extends HTMLElement {
    connectedCallback() {
        ...
        function isActive(names) {
            var active = names.some(function(n) {
                return page === n || path.endsWith('/' + n);
            });
            return active ? 'header__icon--active' : '';
        }
        ...
        this.innerHTML =
            '<header class="header">' +
                ...
                '<nav class="header__nav">' +
                    '<a href="popular.php" class="header__icon ' + isActive(['popular.php']) + '" title="Популярное">' +
                        '<img src="public/assets/icons/Icon_fire.png" alt="Популярное">' +
                    '</a>' +
                    '<a href="search.php" class="header__icon ' + isActive(['search.php']) + '" title="Поиск">' +
                        '<img src="public/assets/icons/Icon_search.png" alt="Поиск">' +
                    '</a>' +
                    '<a href="profile.php" class="header__icon ' + isActive(['profile.php']) + '" title="Профиль">' +
                        '<img src="public/assets/icons/Icon_profile.png" alt="Профиль">' +
                    '</a>' +
                    '<a href="favorites.php" class="header__icon ' + isActive(['favorites.php']) + '" title="Избранное">' +
...
class AuthButtons extends HTMLElement {
    connectedCallback() {
        var serverError = this.dataset.error || '';
            ...
            '<div class="overlay" id="overlay">' +
                '<div class="modal" role="dialog" aria-modal="true">' +
                    ...
                    '<form class="form-panel" id="panel-login" method="POST">' +
                        ...
                        '<div class="form__label">Пароль <span class="required">*</span>' +
        ...
        fetch('public/api/get_csrf.php')
            ...
            .then(function(data) {
                ...
                if (el1) el1.value = data.token;
                if (el2) el2.value = data.token;
            })
            .catch(function() {});
