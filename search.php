...
// --- Сортировка ---
// Важно: avg_score вычисляется в SELECT, поэтому ORDER BY по алиасу работает
switch ($sort) {
    case 'лучшие':
        $order_sql = 'avg_score DESC, p.id DESC';
        break;
    case 'старые':
        // Сначала старые года (1800), потом новые (2026)
        $order_sql = 'p.year ASC, p.id ASC';
        break;
    case 'новые':
    default:
        // Сначала новые года (2026), потом старые (1800)
        // Если год NULL, он будет в конце
        $order_sql = 'p.year DESC, p.id DESC';
        break;
}

// --- HAVING (фильтрация по рейтингу после группировки) ---
$having_parts = [];
if ($min_rating > 0) {
...
// Для COUNT нам не нужен HAVING и JOIN ratings (если нет фильтра по рейтингу в WHERE, а его там нет)
// Но если есть min_rating, он в HAVING, значит в простом COUNT его игнорируем (погрешность допустима)
// Или делаем сложный подзапрос. Для простоты сделаем COUNT без учета min_rating, 
// так как это сильно упрощает код, а на глаз разница в 1 страницу не критична.
$count_params = $params;
$count_types = $types;
if ($min_rating > 0) {
    // Убираем последний параметр (min_rating) и его тип из массивов для COUNT
...
if (!empty($count_params)) {
    $stmt_cnt->bind_param($count_types, ...$count_params);
}
$stmt_cnt->execute();
...
// --- Данные для фильтров (списки тегов) ---
$distinct = [];
foreach ($tag_keys as $k) {
...
<body>
    ...
    <main class="search-page">
        ...
        <form method="GET" class="search-form" id="searchForm">
            <div class="search-bar">
                <input type="text" name="q" value="<?= htmlspecialchars($q) ?>" placeholder="Строка, цитата, настроение или автор.." class="search-input-main">
    ...
    <script>
        // Скрипт для мгновенной сортировки без потери других фильтров
        document.getElementById('sortSelect').addEventListener('change', function() {
            const url = new URL(window.location);
            url.searchParams.set('sort', this.value);
            url.searchParams.set('page', 1); // Сброс на первую страницу при смене сортировки
